...
expected<first_pass_result> first_pass(std::istream& in) {
...
error second_pass(const entity_set& entities, const node_id& nid,
                  const std::vector<string>& json_names,
                  std::istream& in, std::ostream& out,
                  bool drop_hidden_actors) {
      ...
      switch (event.type) {
        ...
        case se_type::receive: {
          ...
          if (i != e) {
            ...
            cerr << "*** ERROR: could not match receive event in line " << line
                 << " to a previous send event" << endl;
          }
          break;
        }
        case se_type::spawn:
        ...
        case se_type::init: {
          ...
          if (i != e) {
            ...
            cerr << "*** ERROR: could not match init event in line " << line
                 << " to a previous spawn event" << endl;
          }
          break;
        }
...
void caf_main(actor_system& sys, const config& cfg) {
  ...
  for (size_t i = 0; i < cfg.args_remainder.size(); ++i) {
    ...
    // spawn actor unless we are dealing with a single file
  ...
  if (!std::is_sorted(entities.begin(), entities.end(), vid_cmp)) {
    cerr << "*** ERROR: entity set not sorted by vector timestamp ID" << endl;
    return;
  }
  ...
  for (auto& fpr : intermediate_results) {
    ...
    out << endl;
    second_pass(entities, fpr.res.this_node, entity_names, fpr.fstream, out,
                !cfg.include_hidden_actors);
  }
}
