...
public:
  stream_serv_class(actor_config& cfg, actor basp, fixture& parent)
      : raw_event_based_actor(cfg),
        basp_(std::move(basp)),
        fixture_(parent),
        incoming_(this, *this, basp_),
        outgoing_(this, *this, basp_) {
    // nop
  }
...
printf("%s %d -- x: %s\n", __FILE__, __LINE__, to_string(current_element_->content()).c_str());
...
printf("%s %d -- x: %s\n", __FILE__, __LINE__, to_string(current_element_->content()).c_str());
        ...
        if (cme->sender != nullptr) {
          ...
          if (outgoing_.has_remote_path(nid)) {
            outgoing_.add_credit(nid, credit);
          } else {
            incoming_.add_credit(nid, credit);
          }
        } else {
