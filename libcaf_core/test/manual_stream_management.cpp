...
public:
  ...
  error downstream_demand(strong_actor_ptr& hdl, size_t new_demand) override;

  error push(size_t* hint) override;

  expected<size_t> add_upstream(strong_actor_ptr& hdl, const stream_id& sid,
                                stream_priority prio) override;

  error upstream_batch(strong_actor_ptr& hdl, size_t, message& xs) override;
...
error stream_governor::add_downstream(strong_actor_ptr& hdl) {
...
printf("initial_demand: %d\n", (int) initial_demand);
  ...
  if (path) {
    ...
    auto res = downstream_demand(hdl, initial_demand);
printf("%s -- local_subs.available_credit: %d, local_subs.buf_size: %d\n",
       __FUNCTION__,
       (int) local_subscribers_.available_credit(),
       (int) local_subscribers_.buf_size());
    return res;
  }
...
error stream_governor::downstream_demand(strong_actor_ptr& hdl, size_t value) {
...
error stream_governor::push(size_t* hint) {
  CAF_LOG_TRACE("");
...
expected<size_t> stream_governor::add_upstream(strong_actor_ptr& hdl,
                                               const stream_id& sid,
                                               stream_priority prio) {
  CAF_LOG_TRACE(CAF_ARG(hdl) << CAF_ARG(sid) << CAF_ARG(prio));
  auto buf_size = local_subscribers_.buf_size();
  auto credit = local_subscribers_.available_credit();
  for (auto& kvp : peers_){
    buf_size = std::min(buf_size, kvp.second->out.buf_size());
    credit = std::min(credit, kvp.second->out.available_credit());
  }
  if (hdl)
    return in_.add_path(hdl, sid, prio, buf_size, credit);
  return sec::invalid_argument;
}

error stream_governor::upstream_batch(strong_actor_ptr& hdl, size_t xs_size,
                                      message& xs) {
  CAF_LOG_TRACE(CAF_ARG(hdl) << CAF_ARG(xs_size) << CAF_ARG(xs));
printf("top of the fun: local_subs.available_credit: %d, local_subs.buf_size: %d\n",
       (int) local_subscribers_.available_credit(),
       (int) local_subscribers_.buf_size());
  using std::get;
printf("%s %d\n", __FILE__, __LINE__);
...
printf("%s %d\n", __FILE__, __LINE__);
  ...
  // Decrease credit assigned to `hdl` and compute the minimum of available
  // credit on all paths as well as the maximum buffer size of paths.
printf("path->assigned_credit: %d\n", (int) path->assigned_credit);
  path->assigned_credit -= xs_size;
  auto available = std::numeric_limits<size_t>::max();
  size_t buffered = 0;
  ...
  for (auto& kvp : peers_)
    if (kvp.first != hdl) {
      ...
      if (out.buf_size() > 0) {
        out.policy().push(out);
        available = std::min(available, out.available_credit());
        buffered = std::max(buffered, out.buf_size());
printf("out.available_credit: %d, out.buf_size: %d\n",
       (int) out.available_credit(), (int) out.buf_size());
      }
    }
...
printf("before push: local_subs.available_credit: %d, local_subs.buf_size: %d\n",
       (int) local_subscribers_.available_credit(),
       (int) local_subscribers_.buf_size());
  local_subscribers_.policy().push(local_subscribers_);
printf("after push: local_subs.available_credit: %d, local_subs.buf_size: %d\n",
       (int) local_subscribers_.available_credit(),
       (int) local_subscribers_.buf_size());
  available = std::min(available, local_subscribers_.available_credit());
  buffered = std::max(buffered, local_subscribers_.buf_size());
  // Grant new credit to upstream if possible.
  if (available > 0)
  {
    in_.assign_credit(buffered, available);
    printf("%s %d\n", __FILE__, __LINE__);
  }
  return none;
}
...
CAF_TEST(two_peers) {
  ...
  auto core2 = sys.spawn(core, filter_type{"c", "d", "e"});
  sched.run();
  ...
  expect((stream_msg::open),
         from(_).to(core1).with(
           std::make_tuple(_, filter_type{"c", "d", "e"}), core2, _, _,
           false));
